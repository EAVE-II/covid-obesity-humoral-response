...
df_cohort_vacc <- readRDS(paste0(project_path,"/data/df_cohort_vacc_13-12-2021_gam_variant.rds"))
df_cohort_vacc_g <- readRDS(paste0(project_path,"/data/df_cohort_vacc_g_13-12-2021_gam_variant.rds"))
...
a_end <- as.Date("2021-09-15")

df_cohort_vacc_g <- df_cohort_vacc_g %>% filter(start_date >= a_end)  
...
glm_pos1 <- glm(event~offset(log(p_years))  +date_period+ Sex + age_gp + vacc_gap+ variant+vacc_type_3+
                  prior_infect_monthgrp1+n_risk_gps + n_tests_gp+simd2020_sc_quintile+
                  ur_combined+bmi_gp + HB , family=poisson,weights = weight,data=df_cohort_vacc_g)
...
for (i in 1:length(conditions)){#length(conditions)
  ...
  z<-cbind(z,round(exp(glm_pos$coefficients-coef(summary(glm_pos))[,2]*1.96),3))
  z<-cbind(z,round(exp(glm_pos$coefficients+coef(summary(glm_pos))[,2]*1.96),3))
  colnames(z)[1]<-"Adjusted Rate Ratio"
  colnames(z)[2]<-"LCI"
  colnames(z)[3]<-"UCI"
