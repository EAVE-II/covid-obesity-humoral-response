...
shielding <- select(shielding, EAVE_LINKNO)
shielding <- mutate(shielding, shielding=1)
df_cohort_vacc_g <- left_join(df_cohort_vacc_g,shielding, by="EAVE_LINKNO")
df_cohort_vacc_g$shielding[is.na(df_cohort_vacc_g$shielding)]<-0
immuno <- select(immuno, EAVE_LINKNO)
immuno <- mutate(immuno, immuno=1)
df_cohort_vacc_g <- left_join(df_cohort_vacc_g,immuno, by="EAVE_LINKNO")
df_cohort_vacc_g$immuno[is.na(df_cohort_vacc_g$immuno)]<-0
######### Modelling
# crude analysis
# sex
...
conditions <- c('shielding','immuno')
ARR <- rbind()
for (i in 2:2){#length(conditions)
  ...
  #summary(glm_pos)
  z<-as.data.frame(round(exp(glm_pos$coefficients),2))
...
write.csv(ARR,paste0(project_path,"/output/ARR_conds_14-28_days_as_ref_immuno.csv"))
...
for (i in 1:length(conditions)){#length(conditions)
  ...
  z<-cbind(z,round(exp(glm_pos$coefficients-coef(summary(glm_pos))[,2]*1.96),2))
  z<-cbind(z,round(exp(glm_pos$coefficients+coef(summary(glm_pos))[,2]*1.96),2))
  z<-cbind(z,round(coef(summary(glm_pos))[,4],3))
  colnames(z)[1]<-"Adjusted Rate Ratio"
  colnames(z)[2]<-"LCI"
  colnames(z)[3]<-"UCI"
  colnames(z)[4]<-"p-value"
  rownames(z)[2]<-conditions[i]
  ARR <- rbind(ARR,z[2,])
  ARR <- rbind(ARR,z[27,])
  ARR <- rbind(ARR,z[28,])
...
ARR<-mutate(ARR, ARR_LCI_UCI = paste(ARR[,1]," (",ARR[,2],"-",ARR[,3],")",sep = ""))
colnames(ARR)[4]<-"ARR (LCI-UCI)"
write.csv(ARR,paste0(project_path,"/output/ARR_interaction_second_booster.csv"))
glm_pos <- glm(event~offset(log(p_years))  +get(conditions[1])+get(conditions[2])+get(conditions[3])+
                 get(conditions[4])+get(conditions[5])+get(conditions[6])+get(conditions[7])+
                 get(conditions[8])+get(conditions[9])+Sex + age_gp + variant +
                 simd2020_sc_quintile + risk_gp + variant*get(conditions[1])+ variant*get(conditions[2])+
                 variant*get(conditions[3])+ variant*get(conditions[4])+ variant*get(conditions[5])+
                 variant*get(conditions[6])+ variant*get(conditions[7])+ variant*get(conditions[8])+
                 variant*get(conditions[9]), 
               family=poisson,weights = weight,data=temp)
###############################