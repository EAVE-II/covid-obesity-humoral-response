...
# Method 2
# df_cohort_vacc_g <- mutate(df_cohort_vacc_g, p_years=as.numeric(tstop)-as.numeric(tstart))
# 
# glm_pos1 <- glm(event ~ offset(log(pyears)) +pv_period_f+period_f + Sex + age_gp+
#                   n_risk_gps+bmi_gp+simd2020_sc_quintile+pos_before_start_u+vacc_gap+
#                   n_tests_gp+ur_combined, 
#                 family=poisson,weight=ew ,data=df_cohort_vacc_g)
...
write.csv(z,paste0(project_path,"/obesity_codes/output/ARR_obesity.csv"))
