...
covid_hosp_death <- filter(covid_hosp_death,admission_date<="2022-03-28")
covid_hosp_death <- filter(covid_hosp_death,SpecimenDate<="2022-03-28")
...
#bmi
df_all <- df_all %>% 
  mutate(bmi_gp = cut(bmi_impute, breaks = c(-1, 18.5, 24.9, 29.9, 34.9,39.9, 51),
...
df_all<-df
df_all <- filter(df_all,is.na(covid_hosp_death_date)|covid_hosp_death_date<="2022-03-28")
###########################
seed <- 39021954
...
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_1463 = 
                         if_else(analysis_end_date_c>(date_vacc_2+63) &
                                   (date_vacc_3_13>(date_vacc_2+63)|is.na(date_vacc_3_13)),
                                 (date_vacc_2+63),
                                 if_else(analysis_end_date_c<(date_vacc_2+63) &
                                           (date_vacc_3_13>(date_vacc_2+63)|is.na(date_vacc_3_13)),
                                         analysis_end_date_c,
...
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_63133 = 
                         if_else(analysis_end_date_c>(date_vacc_2+133) &
                                   (date_vacc_3_13>(date_vacc_2+133)|is.na(date_vacc_3_13)),
                                 (date_vacc_2+133),
                                 if_else(analysis_end_date_c<(date_vacc_2+133) &
                                           (date_vacc_3_13>(date_vacc_2+133)|is.na(date_vacc_3_13)),
                                         analysis_end_date_c,
                       ...
                       date_preiod_133o = 
                         if_else(analysis_end_date_c>(date_vacc_2+133) &
                                 ...
                                 if_else(analysis_end_date_c>(date_vacc_2+133) &
                                         ...
                                         date_vacc_3_13,
                                         if_else(analysis_end_date_c==date_preiod_63133, 
                                                 analysis_end_date_c,date_vacc_3_13))))
...
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_b_1435 = if_else(analysis_end_date_c>(date_vacc_3_13+34),(date_vacc_3_13+34),analysis_end_date_c))
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_b_3556 = if_else(analysis_end_date_c>(date_vacc_3_13+55),(date_vacc_3_13+55),analysis_end_date_c))
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_b_56o = if_else(is.na(date_vacc_3_13),date_vacc_3_13,analysis_end_date_c))
########

df_cohort_vacc_g<-gather(df_cohort_vacc,"date_period","end_date",date_preiod_0014,
                         date_preiod_1463,date_preiod_63133,date_preiod_133o,date_preiod_b_1435,
                         date_preiod_b_3556,date_preiod_b_56o)
...
df_cohort_vacc_g <- mutate(df_cohort_vacc_g, date_period =
                             if_else(date_period=="date_preiod_b_1435" & vacc_type_3=="Mo",
                                     "date_preiod_b_1435_Mo",
                                     if_else(date_period=="date_preiod_b_3556" & vacc_type_3=="Mo",
                                             "date_preiod_b_3556_Mo",
                                             if_else(date_period=="date_preiod_b_56o" & vacc_type_3=="Mo",
                                                     "date_preiod_b_56o_Mo",date_period))))

df_cohort_vacc_g <- mutate(df_cohort_vacc_g, date_period =
                             if_else(date_period=="date_preiod_b_1435" & vacc_type_3=="PB",
                                     "date_preiod_b_1435_PB",
                                     if_else(date_period=="date_preiod_b_3556" & vacc_type_3=="PB",
                                             "date_preiod_b_3556_PB",
                                             if_else(date_period=="date_preiod_b_56o" & vacc_type_3=="PB",
                                                     "date_preiod_b_56o_PB",date_period))))
...
# z<-data.frame(seq(as.Date("2020-12-30"),as.Date("2022-03-11"),"day"))
...
z<-data.frame(seq(as.Date("2020-12-08"),as.Date("2022-03-11"),"day"))
colnames(z)[1]<-"start_date"
z<-mutate(z,variant=ifelse(start_date<"2021-05-19","Alpha",
                           ifelse(start_date<"2021-12-15","Delta","Omicron")))
#WGS_variants_UA <- readRDS("/conf/EAVE/GPanalysis/data/WGS_variants_UA.rds")
df_cohort_vacc_g<-left_join(df_cohort_vacc_g,z,by="start_date")

saveRDS(df_all, paste0(project_path,"/data/df_cohort_vacc_15-03-2022_gam_full.rds"))
saveRDS(df_cohort_vacc, paste0(project_path,"/data/df_cohort_vacc_15-03-2022_gam.rds"))
saveRDS(df_cohort_vacc_g, paste0(project_path,"/data/df_cohort_vacc_g_15-03-2022_gam.rds"))
