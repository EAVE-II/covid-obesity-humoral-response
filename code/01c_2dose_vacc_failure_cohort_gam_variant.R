...
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_2842 = 
                         if_else(analysis_end_date_c>(date_vacc_2+41) &
                                   (date_vacc_3_13>(date_vacc_2+41)|is.na(date_vacc_3_13)),
                                 (date_vacc_2+41),
                                 if_else(analysis_end_date_c<(date_vacc_2+41) &
                                           (date_vacc_3_13>(date_vacc_2+41)|is.na(date_vacc_3_13)),
                                         analysis_end_date_c,
...
df_cohort_vacc<-mutate(df_cohort_vacc,
                       date_preiod_4256 = 
                         if_else(analysis_end_date_c>(date_vacc_2+55) &
                                   (date_vacc_3_13>(date_vacc_2+55)|is.na(date_vacc_3_13)),
                                 (date_vacc_2+55),
                                 if_else(analysis_end_date_c<(date_vacc_2+55) &
                                           (date_vacc_3_13>(date_vacc_2+55)|is.na(date_vacc_3_13)),
                                         analysis_end_date_c,
                       ...
                       date_preiod_56o = 
                         if_else(analysis_end_date_c>(date_vacc_2+55) &
                                 ...
                                 if_else(analysis_end_date_c>(date_vacc_2+55) &
                                         ...
                                         date_vacc_3_13,
                                         if_else(analysis_end_date_c==date_preiod_4256, 
                                                 analysis_end_date_c,date_vacc_3_13))))
...
df_cohort_vacc_g<-gather(df_cohort_vacc,"date_period","end_date",date_preiod_0014,
                         ...
                         date_preiod_56o,date_preiod_b_1428,date_preiod_b_28o)
...
WGS_variants_UA <- readRDS("/conf/EAVE/GPanalysis/data/WGS_variants_update.rds")
colnames(WGS_variants_UA)[2]<-"start_date"
z_event <- WGS_variants_UA %>%
  filter(variant=="Alpha") %>%
  group_by(start_date) %>%
  mutate(num_alpha = n()) %>%
  ungroup()
z_event <- distinct(z_event,start_date,.keep_all = TRUE)
z_event <- select(z_event, start_date,num_alpha)
z <- left_join(z,z_event,by="start_date")
z_event <- WGS_variants_UA %>%
  ...
  ungroup()
z_event <- distinct(z_event,start_date,.keep_all = TRUE)
z_event <- select(z_event, start_date,num_delta)
z <- left_join(z,z_event,by="start_date")
z_event <- WGS_variants_UA %>%
  filter(variant=="Omicron") %>%
  group_by(start_date) %>%
  mutate(num_omicron = n()) %>%
  ungroup()
z_event <- distinct(z_event,start_date,.keep_all = TRUE)
z_event <- select(z_event, start_date,num_omicron)
z <- left_join(z,z_event,by="start_date")
z <- z %>%
  replace(is.na(.),0)
df_cohort_vacc_g<-left_join(df_cohort_vacc_g,z,by="start_date")

#saveRDS(df_cohort_vacc, paste0(project_path,"/data/df_cohort_vacc_13-12-2021_gam_variant.rds"))
#saveRDS(df_cohort_vacc_g, paste0(project_path,"/data/df_cohort_vacc_g_13-12-2021_gam_variant.rds"))
